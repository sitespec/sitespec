...
export function agentProtocol(): Record<string, unknown> {
  return {
    ...
    workflow: {
      ...
      auditExistingSite: "npm run site -- migrate audit <url> --json",
      segmentExistingSite: "npm run site -- migrate segment <audit> --json",
      analyzeExistingSiteDesign: "npm run site -- migrate design <audit...> --json",
      reviewExistingSiteComponents: "npm run site -- migrate components review <analysis> --json",
      proposeExistingSiteComponentContracts: "npm run site -- migrate components contracts <review> --json",
      proposeExistingSiteShellContracts: "npm run site -- migrate shell contracts <component-review> --json",
      reviewExistingSiteUi: "npm run site -- migrate ui review <analysis> --json",
      proposeExistingSiteUiContracts: "npm run site -- migrate ui contracts <review> --json",
      inferExistingSiteImplementations: "npm run site -- migrate implementations infer <analysis> --json",
      stageExistingSiteDesignSystem: "npm run site -- migrate design-system materialize <analysis> --json",
      reviewExistingSiteFoundation: "npm run site -- migrate foundation review <analysis> --json",
      materializeExistingSiteFoundation: "npm run site -- migrate foundation materialize <review> --json",
    ...
    migration: {
      audit: "npm run site -- migrate audit <url> --json",
      segment: "npm run site -- migrate segment <audit> --json",
      design: "npm run site -- migrate design <audit...> --json",
      componentsReview: "npm run site -- migrate components review <analysis> --json",
      componentsContracts: "npm run site -- migrate components contracts <review> --json",
      uiReview: "npm run site -- migrate ui review <analysis> --json",
      uiContracts: "npm run site -- migrate ui contracts <review> --json",
      shellContracts: "npm run site -- migrate shell contracts <component-review> --json",
      implementationsInfer: "npm run site -- migrate implementations infer <analysis> --json",
      designSystemMaterialize: "npm run site -- migrate design-system materialize <analysis> --json",
      foundationReview: "npm run site -- migrate foundation review <analysis> --json",
      foundationMaterialize: "npm run site -- migrate foundation materialize <review> --json",
      output: ".sitespec/audit/<host>/<route>/",
      auditOutput: ".sitespec/audit/<host>/<route>/",
      designOutput: ".sitespec/migration/<host>/design/",
      componentFamiliesOutput: ".sitespec/migration/<host>/design/component-families.json",
      componentReviewOutput: ".sitespec/migration/<host>/design/component-review.json",
      componentContractsOutput: ".sitespec/migration/<host>/design/component-contracts/",
      uiFamiliesOutput: ".sitespec/migration/<host>/design/ui-families.json",
      uiReviewOutput: ".sitespec/migration/<host>/design/ui-review.json",
      uiContractsOutput: ".sitespec/migration/<host>/design/ui-contracts/",
      shellContractsOutput: ".sitespec/migration/<host>/design/shell-contracts.json",
      implementationInferenceOutput: ".sitespec/migration/<host>/design/implementation-inference.json",
      implementationPreviewOutput: ".sitespec/migration/<host>/design/implementation-preview/",
      designSystemStagingOutput: ".sitespec/migration/<host>/design/design-system-staging/",
      foundationReviewOutput: ".sitespec/migration/<host>/design/foundation-review.json",
      foundationTokensOutput: ".sitespec/migration/<host>/design/foundation-tokens.json",
      evidence: ["screenshots", "section crops", "DOM snapshot", "computed design inventory", "media inventory", "leaf UI inventory", "layout evidence", "section candidates", "manual visual segments", "manual production DOM segments"],
      proposals: ["cross-page foundations", "primitive token candidates", "semantic role candidates", "typography roles", "section rhythm", "foundation proposal", "foundation review", "section clusters", "component families", "component review", "component contracts", "leaf UI families", "leaf UI review", "leaf UI contracts", "shell candidates", "shell contracts", "implementation inference", "semantic implementation token extensions", "design system contract staging", "design system implementation staging", "media roles"],
      rule: "Audit representative production pages, select exact live production DOM regions when human review is available, then analyze shared patterns. migrate audit gathers evidence including direct leaf UI observations; migrate segment opens the audited URL in Chrome and records human-confirmed DOM roots plus fingerprints without creating Design System source; migrate design prefers completed manual segments over automatic section candidates, infers section rhythm from section-root vertical padding plus viewport evidence, and writes reviewable foundation/component/UI-family proposals without mutating Design System source; migrate components review records explicit shell/section family architecture decisions while leaving one-off/local families pending; migrate components contracts turns accepted section decisions into reviewable component.yaml proposals; migrate shell contracts turns accepted shell decisions into a default shell-pack contract without fabricating Astro; migrate ui review separates reuse strength from semantic-boundary eligibility so direct Button/Link families may be auto-accepted while heuristic Card/Badge and form-role gaps require explicit review; migrate ui contracts turns accepted leaf UI decisions into schema-valid reviewable ui.yaml proposals without generating implementation or interactive states; migrate foundation review records explicit accept/reject/pending decisions and may preserve an unresolved required layout value as provisional when an existing target/default-template fallback is available; migrate foundation materialize writes a non-destructive token preview and changes canonical design/tokens.json only with explicit --apply --replace; migrate implementations infer turns accepted UI/section/shell contracts plus saved DOM/computed-style evidence into migration-owned Astro implementation previews and additive semantic token extensions, without copying production HTML verbatim or inventing unobserved interaction state; migrate design-system materialize assembles reviewed outputs into contract staging when no implementation inference exists and into design-linted executable implementation staging when it does, while local font binaries and unobserved interactive behavior remain explicit fidelity follow-up."
    },
    assets: {
...
export async function inspectProject(root: string, query?: string): Promise<Record<string, unknown>> {
  ...
  const base = {
    ...
    capabilities: {
      ...
      existingSiteComponentReview: true,
      existingSiteComponentContracts: true,
      existingSiteShellContracts: true,
      existingSiteImplementationInference: true,
      existingSiteDesignSystemStaging: true,
