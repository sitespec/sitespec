...
SiteSpec v0.7 content lives under `content/<collection>/`. A typed collection has a `collection.yaml` manifest and Markdown, YAML, or JSON entries. Keep content selection in Page Spec rather than in Astro components.
...
`design-system.yaml` is the first-class v0.7 Design System contract. Inspect it before visual or structural UI work:
...
Design System packs are source packs, not runtime dependencies. To reuse this system in another v0.7 site, create a portable copy with `npm run site -- design-system pack <directory>` and install it there with `npm run site -- design-system install <directory> --replace`. Installation copies the declared files into the target project; the target remains standalone and owns the installed source.
...
The v0.7 design model has two layers:
...
Local font faces are declared in `design/fonts.yaml`. Font files live under `public/fonts/`. Remote font stylesheets are not part of the v0.7 contract.
...
The component prop contract should accept `urn:site-spec:0.7:type:navigation`. Prefer the existing `navigation-list` section for simple in-page navigation. Never duplicate a cross-site menu into page YAML or component source. Inspect a collection with `npm run site -- spec navigation:<id> --json`.
...
In SiteSpec v0.7, responsive media and production SEO are part of the specification, not project-level Astro configuration.

- Declare local images in typed component/content props with `urn:site-spec:0.7:type:image`. Keep source files under `public/`.
...
The default v0.7 starter deliberately exercises the capabilities described above. Preserve these examples unless the user asks to simplify the project:
...
- `components/pagination` consumes `urn:site-spec:0.7:type:pagination`; page 2 is generated by the blog query rather than maintained as a separate Page Spec.
