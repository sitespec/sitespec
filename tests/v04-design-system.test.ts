...
test("v0.7 starter exposes a first-class Design System contract", async () => {
  ...
  try {
    ...
    assert.equal(designSystem.contractVersion, "0.7");
...
test("v0.7 additive token extensions compile into the site and themes override semantics", async () => {
...
test("v0.7 token extension policy can lock primitive additions", async () => {
...
test("v0.7 sites can select a shell pack from the Design System", async () => {
...
test("Design System packs copy source into another v0.7 site without a runtime dependency", async () => {
